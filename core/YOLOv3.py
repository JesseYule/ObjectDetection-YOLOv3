...
class yolov3(object):
    ...
    def _reorg_layer(self, feature_map, anchors):
        ...
        stride = tf.cast(tf.floor(tf.divide(tf.cast(self.img_size, tf.float32), tf.cast(grid_size, tf.float32))),
                         tf.float32)  # [h,w] -> [y,x] 平均每个网络多少个像素值

        num = tf.add(5, self._NUM_CLASSES)
        # 从最后一个维度将feature_map拆分成三个anchor，每个的shape为(batch_size, cell, cell  ,5 + class_num)
        feature_map1, feature_map2, feature_map3 = tf.split(feature_map, [num, num, num], axis=-1)

        box_centers1, box_sizes1, conf_logits1, prob_logits1 = tf.split(
            feature_map1, [2, 2, 1, self._NUM_CLASSES], axis=-1)  # 分离各个值,在最后一个维度进行

        box_centers2, box_sizes2, conf_logits2, prob_logits2 = tf.split(
            feature_map2, [2, 2, 1, self._NUM_CLASSES], axis=-1)

        box_centers3, box_sizes3, conf_logits3, prob_logits3 = tf.split(
            feature_map3, [2, 2, 1, self._NUM_CLASSES], axis=-1)

        box_centers1 = tf.nn.sigmoid(box_centers1)  # 使得偏移量变为非负,且在0~1之间, 超过1之后,中心点就偏移到了其他的单元中
        box_centers2 = tf.nn.sigmoid(box_centers2)
        box_centers3 = tf.nn.sigmoid(box_centers3)
        ...
        [12,12]
        '''
        # 原代码里要恢复成5x5x1x2 的张量，此处为下面与box_center相乘而恢复成5×5×2的张量
        x_y_offset = tf.reshape(x_y_offset, [grid_size[0], grid_size[1], 2])

        x_y_offset = tf.cast(x_y_offset, tf.float32)

        box_centers1 = tf.add(box_centers1, x_y_offset)  # 物体的中心坐标
        box_centers2 = tf.add(box_centers2, x_y_offset)
        box_centers3 = tf.add(box_centers3, x_y_offset)

        box_centers1 = tf.multiply(box_centers1, stride[::-1])  # 在原图的坐标位置,反归一化 [h,w] -> [y,x]
        box_centers2 = tf.multiply(box_centers2, stride[::-1])
        box_centers3 = tf.multiply(box_centers3, stride[::-1])

        # tf.exp(box_sizes) 避免缩放出现负数, box_size[13,13,3,2], anchor[3,2]
        anchors1 = anchors[0:1]
        anchors2 = anchors[1:2]
        anchors3 = anchors[2:]
        box_sizes1 = tf.multiply(tf.cast(tf.exp(box_sizes1), tf.float32),
                                 tf.cast(anchors1, tf.float32))  # anchors -> [w, h] 使用网络计算出的缩放量对anchors进行缩放
        box_sizes2 = tf.multiply(tf.cast(tf.exp(box_sizes2), tf.float32), tf.cast(anchors2, tf.float32))
        box_sizes3 = tf.multiply(tf.cast(tf.exp(box_sizes3), tf.float32), tf.cast(anchors3, tf.float32))

        boxes1 = tf.concat([box_centers1, box_sizes1], axis=-1)  # 计算除所有的方框在原图中的位置
        boxes2 = tf.concat([box_centers2, box_sizes2], axis=-1)
        boxes3 = tf.concat([box_centers3, box_sizes3], axis=-1)

        boxes1 = tf.expand_dims(boxes1, 3)  # 为下面concat回五维增加一个维度
        boxes2 = tf.expand_dims(boxes2, 3)
        boxes3 = tf.expand_dims(boxes3, 3)
        boxes = tf.concat([boxes1, boxes2, boxes3], axis=3)

        conf_logits1 = tf.expand_dims(conf_logits1, 3)
        conf_logits2 = tf.expand_dims(conf_logits2, 3)
        conf_logits3 = tf.expand_dims(conf_logits3, 3)
        conf_logits = tf.concat([conf_logits1, conf_logits2, conf_logits3], axis=3)

        prob_logits1 = tf.expand_dims(prob_logits1, 3)
        prob_logits2 = tf.expand_dims(prob_logits2, 3)
        prob_logits3 = tf.expand_dims(prob_logits3, 3)
        prob_logits = tf.concat([prob_logits1, prob_logits2, prob_logits3], axis=3)
