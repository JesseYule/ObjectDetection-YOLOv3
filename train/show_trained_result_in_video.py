...
parser.add_argument("--video_id", "-vi", default="testpic/1.mp4", help="传入相机的id,可以是图片,视频,网络摄像头(eg:http://admin:admin@ip:端口/")
# "http://admin:admin@169.254.50.165:8081"
...
graph = tf.Graph()
if flags.model == "cpu":
    input_tensor, output_tensors = utils.read_pb_return_tensors(graph, "checkpoint/yolov3_cpu_nms.pb",
                                                                ["Placeholder:0", "concat_9:0", "mul_6:0"])
else:
    input_tensor, output_tensors = utils.read_pb_return_tensors(graph, "checkpoint/yolov3_gpu_nms.pb",
